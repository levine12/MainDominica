...
<script type="text/javascript">
	...
	$j(document).ready(function() {
		...
		allergyStartDatePicker = new DatePicker("<openmrs:datePattern/>", "allergy_startDate", { defaultDate: parseDateFromStringToJs("<openmrs:datePattern/>", "${today}") });
	...
	function doAddAllergy() {
		...
		$j('#allergy_type').val("");
		allergyStartDatePicker.setDate("${today}");
...
<c:choose>
	<c:when test="${fn:length(allergies) == 0}">
	...
	<div style="width: 100%">
		...
		<c:choose><c:when test="${fn:length(removedAllergies) > 0}">		
...
<c:choose>
	<c:when test="${fn:length(removedAllergies) > 0}">
		<br/>
		<div id="removedAllergyTable" style="display: none">
			...
			<table style="margin: 0px 0px 1em 2em;" cellpadding="3" cellspacing="0" class="alTable">
				...
				<c:forEach var="allergy" items="${removedAllergies}">
					<c:choose>
			...
			<tr>
				...
				<td>
					<select id="allergy_severity">
						<option value=""></option>
					<c:forEach var="allergySeverity" items="${allergySeverities}">
