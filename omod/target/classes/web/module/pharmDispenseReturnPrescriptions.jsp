...
<c:if test="${fn:length(orders) == 0}">
    ...
    <h2>Pharmacy : Outstanding Drug Orders </h2>    
...
<div class="box">
    <form method="post">
        <table cellpadding="5">
            ...
            <th> Approved By </th>
            <th> Approver's Instructions </th>
            </tr>

            <c:forEach var="ord" items="${orders}" varStatus="count">
                <tr>
                    <td valign="top"><a href="portlets/pharmacistDrugCompletion.form?orderId=${ord.orderId}" class="drugList"> <button type="button"> Fill </button> </a></td>
                ...
                <td>${approvingInstructions[count.count-1]}</td>
                </tr>

            </c:forEach>
        </table>
                <div id = "pharmacistEntry">
...
<script type="text/javascript">
    var $j=jQuery.noConflict();                
        $j(document).ready(function() {
                ...
                $j('#pharmacistEntry').dialog({
			autoOpen: false,
			modal: true,
			position: top,
			title: 'Dispense Drug Order',
			width: '60%',                                                
		});
		$j('.drugList').click(function(e) {
                         e.preventDefault();
                         var url=$j(this).attr('href');                            
                         $j('#pharmacistEntry').load(url);
			 $j('#pharmacistEntry').dialog('open');                                                 
		});                
                $j('#closeModal').click(function(e){
                    e.preventDefault();
                    $j('#pharmacistEntry').dialog('close');
                })
